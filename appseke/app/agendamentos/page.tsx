...
export default function AgendamentosPage() {
  ...
  useEffect(() => {
    if (isLoading) return
    if (!isAuthenticated) {
      router.replace("/auth/login?callbackUrl=/agendamentos")
      return
    }
  ...
  const appointmentItems = useMemo(() => {
    return bookings.map((b) => {
      ...
      return {
        ...
        clientName: b.professional_name?.trim() || "Profissional",
      ...
      <div className="mb-5 flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Acompanhe os seus agendamentos e respetivo estado.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {loading && appointmentItems.length === 0 ? (
          <div className="w-full rounded-lg border border-gray-200 bg-white px-6 py-10 text-center text-sm text-gray-600">
            ...
            <div className="mt-3">A carregar agendamentos…</div>
          </div>
