...
/**
 * URL para perfil público `GET /users/:id` na API externa.
 * Usa `API_USERS_BY_ID_PREFIX` (ex. `/api/users`) como em `app/api/users/[id]/route.ts`.
 */
export function buildExternalUserByIdUrl(userId: string): string {
  return getUsersByIdBase(userId)
}
...
export function buildExternalUserSubResourceUrl(
  ...
  return `${getUsersByIdBase(userId)}${sub}`
}
